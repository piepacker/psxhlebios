...
#if HLE_DUCKSTATION_IFC
#define SysErrorPrintf(fmt, ...)    Log_ErrorPrintf(fmt __VA_OPT__(,) __VA_ARGS__)
#define SysPrintf(fmt, ...)         Log_VerbosePrintf(fmt __VA_OPT__(,) __VA_ARGS__)
#define PSXBIOS_LOG(fmt, ...)       Log_VerbosePrintf(fmt __VA_OPT__(,) __VA_ARGS__)
#define PSXBIOS_LOG_SPAM(fmt, ...)  Log_DebugPrintf(fmt __VA_OPT__(,) __VA_ARGS__)
#define PSXBIOS_LOG_IRQ(fmt, ...)  Log_DebugPrintf(fmt __VA_OPT__(,) __VA_ARGS__)
#endif

#if !defined(PSXBIOS_LOG)
#   define PSXBIOS_LOG(fmt, ...) (printf("[HLEBIOS] " fmt "\n" __VA_OPT__(,) __VA_ARGS__), fflush(nullptr))
#endif
